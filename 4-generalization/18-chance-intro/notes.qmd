...
How about if we draw 30 times with replacement from this box and see what our sample looks like (we will explore the code later in the notes). That is, we will see what proportion of the draws are 1's, 2's etc.
...
MUST MOVE THIS PORTION >>>>>>>>>>>
Let's repeat this process, but draw 1000 times instead, and do the same thing (count the number of each ticket, and compute the proportions).
...
set.seed(12345)
sample_size =1000
sample_box2 = sample(box, size = sample_size, replace = TRUE)
data.frame(sample_box2) %>%
  ggplot(aes(sample_box2)) + 
  geom_bar(aes(y = ..prop..), width = 0.98, fill = "grey55") +
  xlab("values of draws") + 
  ylab("proportion of draws")
```

Now the sample proportions look almost identical to the population (box) proportions:
...
|    Ticket | Number of times drawn | Proportion of times drawn |
|:---------:|:---------------------:|:---------------------:|
| $\fbox{1}$| `r table(sample_box2)[[1]]` | `r table(sample_box2)[[1]]/sample_size` |
| $\fbox{2}$| `r table(sample_box2)[[2]]` |`r table(sample_box2)[[2]]/sample_size` |
| $\fbox{3}$| `r table(sample_box2)[[3]]` | `r table(sample_box2)[[3]]/sample_size` |
| $\fbox{4}$|  `r table(sample_box2)[[4]]` | `r table(sample_box2)[[4]]/sample_size` |
...
Remember that the ticket $\fbox{1}$ represents the outcome of a toss landing heads and  $\fbox{0}$ represents a toss landing tails (because we are counting the number of heads). We let the box be as above, and draw two tickets *with replacement* from this box. To count the number of heads in two tosses, we add the draws.
</details>
...
<details><summary>Check your answer</summary>
We would use the same box as in the previous example, draw *twice* with replacement from this box, then add the draws.
...
<details><summary>Check your answer</summary>
Use the same box as above, with 18 tickets marked $\fbox{+1}$ and 20 marked $\fbox{-1}$, draw 10 times with replacement, and sum the draws.  
</details>
